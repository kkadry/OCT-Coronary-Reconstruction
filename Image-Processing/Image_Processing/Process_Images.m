...
plaque=importdata('wall_class_output_withSubpart_P11.txt');
points=plaque(:,1:3);
...
%     scatter(plaque_frame(:,1),plaque_frame(:,2))
%     imagesc(slice)
%     axis([0 200 0 200])
%     pause(0.1)
%     if frame == frames(end)
...
%                     pause(0.1)
    ...
    % Convert back from pixel to real dimesions
%     v(:,1) = (v(:,1)-10).*dx+min(plaque_frame(:,1));
%     v(:,2) = (v(:,2)-10).*dx+min(plaque_frame(:,2));
    v(:,1) = (v(:,1)-10).*dx+min(points(:,1));
    v(:,2) = (v(:,2)-10).*dx+min(points(:,2));
    v(:,3) = z(frame);
    ...
    if A1 < A2
        ...
        %Vpush=push_boundaries(FV1.vertices,FV2.vertices, min_wall_thickness);
        ...
        Frame{frame}.Inner.geom =lumprop;
        %k
        %patch(FV2.vertices(:,1),FV2.vertices(:,2),FV2.faces)
        %pause(0.1)
    end
